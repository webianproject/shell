...
function registerWindowEventListeners(windowId) {
	...
	url_input.keydown(function(event){
		if(event.keyCode == '13'){
			...
			if(!urlRegex.test(val)){
				$(this).val('http://www.google.co.uk/search?q=' + val);
				$(this).parent().submit();
			}
		}
	});
	...
	// Back
	our_window.find(".back_button").click(function() {
		if (urlHistory[windowId][0] < 2) {
			return;
		}
	...
	// Forward
	our_window.find(".forward_button").click(function() {
		if(urlHistory[windowId][0] + 1 >= urlHistory[windowId].length) {
			return;
		}
		$("#windows .selected .window_iframe").attr("src", urlHistory[windowId][++urlHistory[windowId][0]]);
		urlHistory[windowId][0] = urlHistory[windowId][0];
...
function activateWindows() {
	$("#home_screen.active").removeClass("active");
